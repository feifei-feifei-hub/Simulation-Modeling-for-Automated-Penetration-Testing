...
import dill
from graph import iiiiGraph, Graph_test
from model import *
from warnings import filterwarnings
#from sklearn.metrics import f1_score
from utils import iiiiconfigure_link, mean_reciprocal_rank, ndcg_at_k
...
class Trainer:
    def __init__(self,  args,gpu=0):
        ...
        self.device = torch.device("cuda:"+ str(gpu)) if args['cuda']!=-1 else torch.device("cpu")
        self.graph = iiiiGraph(args, self.device)
        self.gnn = GNN(in_dim=self.graph.node_feature[args['cccc']].shape[-1], n_hid=self.n_hid, num_types=1, num_relations=2, n_heads=self.n_heads, n_layers=self.n_layers,
    ...
    def train(self,args):
        ...
        for epoch in np.arange(self.n_epoch)+1:
            ...
            train_datas = []
            valid_pairs_all = []
            for c in range(args['cccc'],args['cccc']+1):
            ...
            for train_data in train_datas:
                for node_feature, node_type, edge_time, edge_index, edge_type, x_ids, ylabel, samp_nodes in train_data:
                    ...
                    if type(node_rep) == dict:
                        node_rep = node_rep['def']
            ...
            with torch.no_grad():
                ...
                if valid_AUC > best_val:
                    best_val = valid_AUC
                    torch.save(self.model, os.path.join(args['model_dir'],'cc1212real'+'+'+ args['task_name']+ '+' + str(args['use_pretrain']) + '+' + args['conv_name']+'+'+str(args['n_epoch'])+'+'+args['premodel']))
...
#AUC
    ...
    @torch.no_grad()
    def test(self,args):
        best_model = torch.load(os.path.join(args['model_dir'], 'cc1212real'+'+'+args['task_name']+ '+' + str(args['use_pretrain']) + '+' + args['conv_name']+'+'+str(args['n_epoch'])+'+'+args['premodel'])).to(self.device)
        best_model.eval()
        ...
        print(f'std:'+'%.3f' % np.std(test_res))
        print(f'var:'+'%.3f' % np.var(test_res))
...
if __name__ == '__main__':
    gpu = random.choice([0,1,2,3,4,5,6,7])
    ...
    logfile = logging.FileHandler('test_result.txt', 'w')
    logfile.setFormatter(fmt); logger.addHandler(logfile)
    try:
        tuner_params = nni.get_next_parameter()
        args = vars(merge_parameter(iiiiconfigure_link(), tuner_params))
        args['use_pretrain'] = bool(int(args['use_pretrain']))
        #args['use_pretrain'] = False
        #model_name = 'newselfpre+hgt+128+800+0.01+40+3+1+32+5+8.pk'
        model_name = args['premodel']
        args['conv_name'] = model_name.split('+')[1]
        #args['conv_name'] = 'han'
        args['batch_size'] = int(model_name.split('+')[2])
        args['n_hid'] = int(model_name.split('+')[3])
        #args['n_epoch'] = 10
        args['n_layers'] = int(model_name.split('+')[6])
        args['sample_depth'] = int(model_name.split('+')[7])
        args['sample_width'] = int(model_name.split('+')[8])
        args['n_batch'] = int(model_name.split('+')[9])
        args['n_heads'] = int(model_name.split('+')[10].split('.')[0])
        args['pretrain_model_dir'] = f'../models/pre_model/{model_name}'
        #args = configure()
        logger.info(args)
        final_score = []
        for i in range(5):
            print('----------------------------------------------------------------')
            print('i',i)
            trainer = Trainer(args,gpu)
            trainer.train(args)
            res=trainer.test(args)
            final_score.append(res)
        res_ = np.average(final_score)
        print('finnal_score:',final_score)
        print('finnal_score_mean:',np.average(final_score))
        print('finnal_score_std:',np.std(final_score))
        print('finnal_score_var:',np.var(final_score))

    except Exception as exception:
        logger.exception(exception)
        raise Exception
    nni.report_final_result(res_)
    torch.cuda.empty_cache()
    # args = configure_link()
