...
for c in range(17):
    ...
    # #np.save("eval/data/edge_list_",graph_net.edge_list['def']['def']['def'])
    # with open("eval/data/edge_list_.json","w", encoding='utf-8') as f: ## 设置'utf-8'编码
    #     f.write(json.dumps(el,ensure_ascii=False))
    # np.save("eval/data/test_nodes_",graph_net.test_target_nodes)
    # np.save("eval/data/train_nodes_",graph_net.train_target_nodes)
    # np.save("eval/data/valid_nodes_",graph_net.valid_target_nodes)
    # np.save("eval/data/X_",x)
    # np.save("eval/data/Y_",(graph_net.y.numpy()))
