...
class GPT_GNN(nn.Module):
    def __init__(self, gnn, rem_edge_list, attr_decoder, types, neg_samp_num, device, neg_queue_size = 0):
        ...
        self.init_emb = nn.Parameter(torch.randn(gnn.in_dim))#torch.
    ...
    def feat_loss(self, reps, out):
        return 10-self.attr_decoder(reps, out).mean()
...
class GNN(nn.Module):
    ...
    def forward(self, node_feature, node_type, edge_time, edge_index, edge_type):
        ...
        for t_id in range(self.num_types):
            ...
            if idx.sum() == 0:
                continue
            res[idx] = torch.tanh(self.adapt_ws[t_id](node_feature[idx]))#torch.tanh()
